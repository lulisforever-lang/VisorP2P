...
def editar_ajuste_dialog(c_num, aj_actual, u_gan_actual, cap, f_h, f_ini_val="", f_fin_val="", ret_admin_actual=0.0):
    ...
    with c_ini_box:
        ...
        d_ini = st.date_input("Fecha Inicio:", value=dt_ini_parsed.date(), key=f"dlg_d_ini_hg_{c_num}")
        t_ini = st.time_input("Hora Inicio:", value=dt_ini_parsed.time().replace(second=0, microsecond=0), key=f"dlg_t_ini_hg_{c_num}", step=60)
    with c_fin_box:
        st.markdown("<div style='font-size:0.85rem; font-weight:600; color:#58a6ff; margin-bottom:4px;'>🔴 Fin:</div>", unsafe_allow_html=True)
        d_fin = st.date_input("Fecha Fin:", value=dt_fin_parsed.date(), key=f"dlg_d_fin_hg_{c_num}")
        t_fin = st.time_input("Hora Fin:", value=dt_fin_parsed.time().replace(second=0, microsecond=0), key=f"dlg_t_fin_hg_{c_num}", step=60)
    ...
    with c_e2:
        nuevo_ret_adm = st.number_input(
            ...
            format="%.2f",
            min_value=0.0,
            key=f"input_modal_ret_adm_hg_{c_num}",
